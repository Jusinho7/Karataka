...
func _process(_delta: float) -> void:
	...
	# Si on est dans la banque ou chez Rasoa, on s'assure que la musique est stoppée
	if "main_scene_bank.tscn" in current_scene or "intro.tscn" in current_scene:
		if _music_player.playing:
			stop_music()
	else:
		# Dès qu'on est sur la map principale (ou ailleurs), si la musique ne joue pas, on la relance
		if not _music_player.playing and _current_music_path == "":
			play_music(MENU_MUSIC_BASENAME)
