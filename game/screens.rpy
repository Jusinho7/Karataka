...
style window:
    ...
    xfill True
    yalign 1.0
    ysize 195   # un peu plus grand pour effet parchemin

    background Frame("gui/parchment_box.png", 60, 60)
...
style say_dialogue:
    ...
    xpos gui.dialogue_xpos -40   # décale un peu à droite
    ypos gui.dialogue_ypos -30  # descend légèrement
    xsize gui.dialogue_width -50 # réduit largeur pour éviter les bords
...
screen navigation():
    """frame:
        background "#00000080"  # noir transparent
        xpadding 10
        ypadding 10
    """
    ...
    vbox:
        ...
        if main_menu:
            ...
            textbutton _("Nouvelle partie"):
                if main_menu and renpy.get_screen("main_menu"):
                    text_size 42 # taille actuelle (tu gardes ton style)
                    ysize 100
                else:
                    text_size 26  # taille réduite pour autres screens
                    xsize 276
                    ysize 90
                action Start()
...
style navigation_button:
    ...
    background "gui/button/wood_idle.png"
    hover_background "gui/button/wood_hover.png"
...
screen game_menu(title, scroll=None, yinitial=0.0, spacing=0):
    ...
    frame:
        ...
        # 🔹 TA CORDE (SUPERPOSÉE)
            add "gui/overlay/rope.png":
                xpos 69  # 👈 position EXACTE de la séparation
                yalign 0.0
                ysize 650
    ...
    textbutton _("Retour"):
        ...
        xsize 300
        ysize 90
        xalign 1.0    # pousse le bouton complètement à droite
        yalign 0.0    # aligne le bouton en haut
        xoffset -20   # petit décalage depuis le bord droit (optionnel)
        yoffset 10    # petit décalage depuis le haut (optionnel)
        action Return()
...
style game_menu_label_text:
    size 50
    color "#f5deb3"   # beige bois

    outlines [
        (3, "#b67219", 0, 0),   # contour sombre
        (1, "#0ccc1c", 1, 1)    # petite ombre décalée
    ]
    yalign 0.5
    xpos 440 
...
style window:
    ...
    xfill True
    yalign 1.0
    ysize 250   # plus petit pour écran mobile

    background Frame("gui/parchment_box.png", 40, 40)
...
#boutton en bois
############################################################
# STYLE BOUTONS BOIS (MENU PRINCIPAL)
############################################################

# Bouton (plaque en bois)
style navigation_button:
    background Frame("gui/wood_button.png", 20, 3)
    hover_background Frame("gui/wood_button_hover.png", 20, 3)
    #xpadding 40
    #ypadding 20
    xminimum 400
...
style navigation_button_text:
    ...
    xalign 0.5
#####################################
# Le screen pour la scene_1         #
#####################################
"""screen scene1_interactions():

    # Fond cliquable
    imagemap:
        ground "images/bg/ferme_vide.png"
        hover "images/bg/ferme_vide.png"

        # =========================
        # 🟡 ZONE POCHES
        # =========================
        hotspot (800, 500, 200, 200) action Jump("check_poches")

        # =========================
        # 🟡 ZONE POULAILLER
        # =========================
        hotspot (300, 450, 250, 200) action Jump("check_poulailler")

    # UI texte (optionnel)"""
    #text "Argent: [argent] Ar" xpos 20 ypos 20 size 30 color "#ffffff"

screen scene1_interactions():
    add "images/bg/ferme_vide.png"

    # 🟡 POCHES
    imagebutton:
        idle Transform("images/ui/highlight_poches.png", size=(120, 120))
        hover Transform("images/ui/poches.png", size=(120, 120))
        xpos 550
        ypos 500
        action Jump("check_poches")

    # 🟡 POULAILLER
    imagebutton:
        idle "images/ui/transparent.png"
        hover "images/ui/highlight_poulailler.png"
        xpos 856
        ypos 124
        action Jump("check_poulailler")

    text "Argent: [argent] Ar" xpos 20 ypos 20 size 30 color "#ffffff"
        # ✅ OPTION BANQUE (s'affiche seulement si tout est vu)
    if a_regarde_poches and a_regarde_poulailler:

        textbutton "🏦 Aller à la banque":
            xpos 0.5
            ypos 0.0
            xanchor 0.5
            ...
            background Frame("#1e1e2ecc", 10, 10)
            hover_background Frame("#3a3a5acc", 10, 10)

            text_size 32
            text_color "#ffffff"
            text_hover_color "#ffd700"

            action Jump("scene_banque")